...
public class CmdPaystubList extends Subcommand {
	...
	private void showRequests(CommandSender s, PaginatedList<PayRequest> requests, int page) {
		...
		s.sendMessage(requests.getFooter(page, "/paystub requests " + (page+1), "/paystub requests " + (page-1)));
	}
	
	private void buildRequest(ComponentBuilder b, PayRequest req) {
		...
		.event(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/paystub deny " + req.getId()));
	}
}
